...
def fitting():
	...
	if desired_thickness - fitted_values[0] < 5:
		...
		finish = True
		np.save('real', real_data_history)
		np.save('fitted', fitted_history)
		t1.join()
...
window = sg.Window('Realtime_PAOO', final_layout, finalize=True,
                   resizable=True, auto_size_text=True, debugger_enabled=False)
...
while True:
	...
	if event == 'SAVE-PLOTS':
		...
		save_path = Path(sg.popup_get_folder('asd', no_window=True))
		if save_path:
			window.close()
