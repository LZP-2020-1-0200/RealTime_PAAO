...
from RealTime_PAAO.common.constants import (
    ...
    SPECTRUM_Y_LIM,
    SPECTRUM_Y_TICKS,
    THICKNESS_PER_TIME_TITLE,
    THICKNESS_PER_TIME_X_LABEL,
    THICKNESS_PER_TIME_Y2_LABEL,
    THICKNESS_PER_TIME_Y_LABEL,
)


def save_fitting_data(
    list_of_spectrum_files: list,
    all_real_data: list[list],
    all_fitted_data: list[list],
    plot_folder,
    calculated_data_folder,
    window,
):
    fig, ax = plt.subplots()
    ...
    for i, (spectr, real_data, fitted_data) in enumerate(zip(list_of_spectrum_files, all_real_data, all_fitted_data)):
        ...
        df = pd.DataFrame({"Wavelength (nm)": LAMBDA, "Experimental data": real_data, "Fitted data": fitted_data})
...
def save_thickness_per_time_data(thickness_hist, thickness_time, path_to_save):
    ...
    thick_per_time = pd.DataFrame({"Time(s)": thickness_time, "Thickness(nm)": thickness_hist})
...
def save_current_per_time_data(current_dict: dict, path_to_save: Path):
    fig, ax = plt.subplots()
    ax.plot(current_dict.keys(), current_dict.values(), label="Current per time")
    ax.set_xlabel(THICKNESS_PER_TIME_X_LABEL)
    ax.set_ylabel(THICKNESS_PER_TIME_Y2_LABEL)
    ax.set_title("Current dependence on anodization time")
    ax.legend()
    ax.grid()
    fig.savefig(path_to_save / "Current_per_time.png")

    curr_per_time = pd.DataFrame({"Anod Time (s)": current_dict.keys(), "Current (nm)": current_dict.values()})
